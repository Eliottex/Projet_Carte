    ...
    for x in range(len(liste_num)):
        for y in range(len(liste_num[x])):
            ...
            if nbr_car * num_colon + num_ligne < len(liste_tuiles):
                fltk.image(y * decalage_larg + 105, x * decalage_haut + 105, "tuiles/" + liste_tuiles[nbr_car * num_colon + num_ligne]+".png", decalage_haut - 10, decalage_larg - 10, ancrage='nw', tag='image' + str(num_ligne) + str(num_colon))
...
def tuiles_possibles(grille,i,j): # i -> colonne; j -> ligne;
    ...
    if gauche=='R' or droite=='R' or haut=='R' or bas=='R':
        ...
        list_valide=[]
        for z in range(len(list_intermediaire)-1):
...
def solveur2(grille):
    ...
    for tuile in tuiles:
        ...
        #affichage FLTK
        fltk.image(j*largeur_carreau,i*hauteur_carreau,"tuiles/" + tuile + ".png", hauteur_carreau, largeur_carreau, ancrage = 'nw', tag = 'image'+str(j)+str(i))
        fltk.mise_a_jour()
        time.sleep(0.01)
        #fin affichage FLTK
        if solveur2(grille):
