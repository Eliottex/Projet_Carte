...
def bouton_retour(hauteur_fenetre,largeur_fenetre):
    
    fltk.rectangle(largeur_fenetre-30, 0, largeur_fenetre, 30, "red", "red")
    fltk.texte(largeur_fenetre-25,0,'X',police=10)
    fltk.mise_a_jour()
...
def tuiles_possibles(grille,i,j, clic=False): # i -> colonne; j -> ligne;
    ...
    clic                    -Permet de savoir si la fonction est pour le solveur ou le joueur
   ...
   #Gestion des rivières liées à l'océan
    ...
    if lst_finale!=[] or clic!=True:
        return lst_finale
    else:
        return list_intermediaire
                ...
                if grille_dec[x][y]==None:
                    ...
                    if b in [x for x in range(5)]:
                        ...
                        if list_image [x][y][3] == 'H' and list_image[x][y][2] == 'P'and list_image[x][y][1] == 'H':
                            ...
                            if zzzz=='village_v2.png':
                                grille_dec[x][y][2] = 2.5
            ...
            if tev == "ClicGauche":
                if mode == 'choisir_case':
                    ...
                    if fltk.ordonnee_souris()<=30 and fltk.abscisse_souris()>=largeur_fenetre-30:
                        ...
                        if case!=None:
                            fltk.rectangle(case[1]*largeur_carreau,case[0]*hauteur_carreau,(case[1]+1)*largeur_carreau,(case[0]+1)*hauteur_carreau,'lightgreen','lightgreen')
                            bouton_indice()
                            bouton_retour(hauteur_fenetre,largeur_fenetre)
                    else:
                        list_valide = tuiles_possibles(list_image,num_col,num_lig,True)
